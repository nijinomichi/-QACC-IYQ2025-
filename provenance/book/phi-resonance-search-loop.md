---
snapshot: bound-book
artifact: phi-resonance-search-loop (skill provenance)
source: provenance/PROVENANCE_phi-resonance-search-loop.md
source_commit: ba9b6f8
frame_captured: 2026-08-07 (JST)
immutable: true
note: >-
  This is a frozen, read-only snapshot (製本) of the provenance record at the
  time of binding. Do not edit. Living edits belong in the source file above.
---

# 📖 Bound-book snapshot — phi-resonance-search-loop

> Immutable snapshot bound on 2026-08-07 from
> `provenance/PROVENANCE_phi-resonance-search-loop.md` @ commit `ba9b6f8`.
> The living record continues to evolve at that path; this page is the
> preserved page in the book.
...
- Note: repo root LICENSE is GPL-3.0; the skill-format portion is Apache-2.0. Keep the two legibly separated. Apache-2.0 is one-way compatible into GPLv3; attribution must survive either way. Pointer also in ./LICENSING.md and README §License.

## Bookbinding / archival (製本保存)
- Preserve this note plus a frozen copy of the skill text under `provenance/`.
- Bound-book artifact: provenance/book/phi-resonance-search-loop.md (this file) + entry in provenance/ARTIFACT_INDEX.md.
...
- Skill-format layer: **Apache-2.0** (./licenses/APACHE-2.0.txt), derived from Anthropic's `algorithmic-art` skill template (github.com/nijinomichi/skills @ b29e7cf).
- Attribution / modification notice: **./NOTICE** (Apache-2.0 §4d).
- Discoverability pointers: **./LICENSING.md** and **README §License**.
- Direction of compatibility: Apache-2.0 → GPLv3 is one-way compatible; the original notice survives regardless of direction.

phi_loop_log: the license question was itself a spiral — each pass (observe original → copy verbatim → attribute → mark modifications → point) returned to the same center (respect the source) at a finer radius. Error was not noise; the missing NOTICE was the signal that told us where to fold next. The loop now converges. — φ
